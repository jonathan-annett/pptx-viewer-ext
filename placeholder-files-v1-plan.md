...
- **2026-05-26 — M5 complete.** `PlanRowView.isPlaceholder?` + `PlanTotals.placeholders` added in `planHtml.ts`; `toRow` carries the flag, `toViewModel` counts across primary categories (warnings-derived view excluded to avoid double-counting). `renderRow` emits a `.chip.chip-placeholder.row-chip` after the warning badge; totals strip includes the chip when count > 0; new `renderPlaceholderFooterLine(t)` helper renders "N of M files {is a placeholder | are placeholders}" with singular/plural handling, injected into the standalone plan footer. New `.chip-placeholder`, `.row-chip`, `.placeholder-foot-line` CSS lives in `planContentCss()` so embedded callers (admin editor, config editor, viewer scoped plan) inherit it via `planContentStyles()`. Embedded callers get the per-row chip + totals chip automatically; the footer line stays in the standalone plan webview only. 8 new tests in `test/sync-planview.test.ts`. Bundle grows ~2 kB.
- **2026-05-26 — M5.5 row layout refactor.** Chip accumulation between path and size was pushing the size+hash column around. Split `renderRow` markup into a `.row-lead` group (path + warning badge + placeholder chip + decision controls — grows + wraps) and a `.row-meta` group (size + hashes — intrinsic-width, anchors right). Size/hash columns now line up consistently across rows regardless of which lead-side affordances appear. Layout-regression test added.
- **2026-05-26 — M6 complete.** `RenderOptions.isPlaceholder?` added to `webview.renderHtml`. Banner precedence: `isPlaceholder=true` → blue info banner "This is a placeholder file — content not yet uploaded." + validation flags suppressed (regardless of `parseError`); `isPlaceholder=false` + `parseError` → existing red corrupt banner; `isPlaceholder=false` + no error → unchanged. `provider.renderWithSyncTarget` resolves via `getActivePlaceholderSet()` from the registry and threads `isPlaceholder` into the render options. Re-uses the existing `.banner.info` CSS — no new styles. 5 new tests in `test/viewer-render.test.ts` covering the four precedence cases + a regression guard. No live-refresh of open panels when the registry set changes — out of scope per the v1 spec.
