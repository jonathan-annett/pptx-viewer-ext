...
- **Webview CSP is explicit.** The webview HTML carries its own `<meta http-equiv="Content-Security-Policy">`. The current policy is `default-src 'none'; style-src 'unsafe-inline'; img-src data:;`. When adding a new resource type to the panel (fonts, scripts, external images), widen the policy in `src/webview.ts` deliberately, not by removing it.
