...
> **Maintainer / contributor notes.** End-user documentation lives in
> [`README.md`](README.md); this file covers building, testing,
> publishing, and the implementation-flavoured detail that's useful when
> working on the extension itself but irrelevant to someone just
> installing it.
...
1. **Pptx viewer** — a read-only viewer for `.pptx` files that surfaces
   ...
   rendering — by design.)
2. **Folder sync** — a one-way folder sync for vscode.dev: read access on
   a source folder tree, write access on one or more destination folders,
   user-convened with a plan-gate-execute model. The pptx viewer's
   validation checks plug into the sync as flagged items.
...
Currently shipped (milestones M1 through M6 plus M-VE-1/2/3 viewer tracks):
...
- Schema validation and topology resolution (matching destination URIs to
...
- Custom editor for `.sync.jsonc` with form-style destination/path/glob
  controls and an embedded scoped dry-run
- Plan engine + categorised plan webview + executor with manifest writes
- Status bar item, Explorer right-click menu, command palette invocation
- Interactive decision UI (collisions, destination-only, validator
  warnings, "don't ask again" persistence)
- Workspace snapshot (`.admin-sync.jsonc`) + silent restore on browser
  refresh
- Custom editor for `.foldersync-manifest.json` (view-only tabular)

See `folder-sync-v1-plan.md` at the repo root for the full milestone
record.
