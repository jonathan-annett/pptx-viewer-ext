...
## Deferred follow-ups
- **Batch the parse-cache IDB reads into a single `getAll()`** (committed during the v1 cycle, deferred to next). Profiling during the placeholder sign-off showed a ~2.5s delay on every viewer open, caused by the per-file scoped sync-target build looping `parseCache.lookup(sha)` for each source file — even with all hits warm, that's 24 IDB round-trips for a typical workspace. The placeholder feature shipped a partial mitigation for the zero-byte path only (`parsePptxCached` short-circuits before the IDB lookup; see `src/sync/parseCache.ts`). The general fix: at walk start in the planner / indexer, call a single `parseCache.getAll()` (or batched `getMany([sha1, sha2, …])`) into an in-memory map, then satisfy per-file lookups from there. Same shape would benefit `hashCache` if profiling shows it's also IDB-bound. Cross-cutting perf, not placeholder-specific — affects search-panel walks and the planner equally. Pick this up before the next folder-sync milestone or any other feature work.
