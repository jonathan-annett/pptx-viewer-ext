...
### M5 — Button + modal wiring *(✅ shipped 2026-05-24)*

- **`src/webview.ts`** — Action row carries "Browse to Update…" + "Upload to Update…" buttons. `uploadBtn.click` posts `{type:'uploadOpen'}`. `openUploadModal(html)` is the sibling of the existing `openModal()` and binds the upload modal's four well-known ids (`upload-cancel-btn`, `upload-close-btn`, `upload-retry-btn`, `upload-copy-btn`). Message-listener cases for `uploadModal` (re-render contents in place), `uploadModalClose` (dismiss + clear status), and `uploadedBytes` (PDF magic-bytes / filename routing → `handlePdfFile` or `{type:'ingest', source:'upload'}` back to host).
- **`src/upload/uploadFlow.ts`** — One `UploadFlow` per session. Holds the state-machine + 1 Hz countdown timer + per-transition full-modal re-render. `cancel()`/`close()`/`retry()`/`dispose()` are all idempotent. On `complete` it posts the bytes back to the webview via the provider's `onComplete` callback and intentionally does **not** auto-dismiss the modal — the webview owns lifecycle from that point so it can swap to the PDF-import modal without a race.
- **`src/upload/baseUrl.ts`** — Shared `resolveDropboxBaseUrl()` used by both `uploadFlow.ts` and `probeUpload.ts`. Reads `pptxViewer.dropboxBaseUrl`, defaults to `https://vscode.sophtwhere.com/dropbox`.
- **`src/provider.ts`** — Per-panel `currentUploadFlow` slot (disposed in `onDidDispose` and on each new `uploadOpen`). Message handlers for `uploadOpen` / `uploadCancel` / `uploadClose` / `uploadRetry`. `handleIngest` accepts `'upload'` as a third source value, routed identically to `'picker'`: parse-cache `forget(sha)` so a re-upload of identical content reparses fresh, then `writeAndRender`, then `{type:'picker-result', outcome:'updated'}`.
- DoD verification deferred to the user's live walkthrough; bundle is rebuilt locally (`dist/extension.js` 447 KB + 1.5 MB inlined webview).
