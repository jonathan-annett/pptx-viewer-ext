...
export class SyncConfigEditorProvider implements vscode.CustomTextEditorProvider {
  static register(): vscode.Disposable {
    const provider = new SyncConfigEditorProvider();
  ...
  async resolveCustomTextEditor(
    ...
    panel.webview.onDidReceiveMessage(async (msg: WebviewMessage) => {
      try {
        ...
        } else if (msg.type === 'requestDryRun') {
          // Reuse the existing plan command. M4.5 keeps things minimal: open
          // the workspace-wide plan in a separate panel rather than embedding
          // it. A future polish task can inline the plan into this editor.
