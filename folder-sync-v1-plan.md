...
- **Phase D — Manifest version-mismatch refusal *(shipped 2026-05-23, commit `6c828a7`)*.** `readManifest` now returns a discriminated `ManifestReadResult` (`{ kind: 'ok', manifest }` / `{ kind: 'version-mismatch', actual }`). Planner surfaces mismatches as `skippedReason`; `runSync` collects them into `summary.manifestVersionMismatches` and skips the whole group. Both green-path proceed surfaces (workspace plan panel + per-file sync from the viewer) call the new exported `surfaceManifestVersionMismatches()` in `planView.ts`, which shows a warning toast with `Open Manifest` / `Show Details` actions. Writing v1 over an unknown schema would clobber the user's prior tracking, so refusal is the safe default.
- **Phase E — Manifest custom editor *(shipped 2026-05-23, commits `6c828a7`, `bce7fe6`)*.** `folderSync.manifestEditor` registered against `**/.foldersync-manifest.json`, priority `default`. View-only tabular renderer: header (version + lastSync + dest root), sorted entries table (key / dest path / humanised size / sha-first-12 with tooltip / relative-time synced-at), sorted decisions table (✓/– flag glyphs + decided-at), version-mismatch banner replacing tables when `readManifest` returns the mismatch variant, Reopen-as-text escape hatch. Pure renderer + view model in `src/sync/manifestEditorHtml.ts` (covered by `test/sync-manifest-editor-html.test.ts` — 14 cases including helpers, sorting, ok/mismatch variants, CSP nonce, empty state, table contents, flag glyphs, version-mismatch banner, HTML-escape). Wired `ManifestEditorProvider` in `src/sync/manifestEditor.ts` re-renders full HTML on `onDidChangeTextDocument` so the executor's mid-sync writes refresh the editor live. The parse helper was extracted into `manifest-types.ts` as `parseManifestText` + `normaliseManifest` so the editor and `readManifest` share one parser (mirrors the `parseSnapshot` / `parseSyncConfigText` pattern). CSP: same locked-down shape as the other custom editors; one nonced inline script for the Reopen handler.
- **Phase F — DoD walkthrough + sign-off *(shipped 2026-05-23)*.** All 9 v1 Definition-of-Done bullets verified against the code: authoring + invocation surfaces, workspace and folder-scoped plan builds, plan webview categories + traffic light, green/orange/red gate, decision persistence, manifest create/update/read + survive-move, per-file failure isolation, pptx validator as a plan category, web-only platform constraints + atomic writes. `CLAUDE.md`'s "What's currently shipping" updated with a new bullet covering all six M6 phases; the substrate doc now reflects v1 as feature-complete.
...
### M-VE-1 — PDF → PPTX import *(shipped 2026-05-23)*
...
- **Phase E — VPS validation + sign-off.** *Done.* `npm install` ran on the VPS to pick up `pdfjs-dist`, `pm2 restart pptx-watch` rebuilt with the new two-bundle config, PWA validated end-to-end. Five sign-off changes landed during Phase E itself (`3b73ec9` pdfjs-v5 fake worker, `f64f794` in-file thumbnail, `553fa01` fixed-height thumbnails + import-time cache eviction, `b691c93` log-line rewording, plus the substrate commit) — captured as Phase D-fix / D-thumbnail / D-followup entries below. Bundle-size note post-fixes: `dist/extension.js` is ~2.0 MB (was ~366KB pre-M-VE-1) — the +1.6 MB is the inlined webview bundle (the +1.2 MB delta vs the original Phase D ~431KB came from inlining `pdf.worker.min.mjs` to satisfy pdfjs-v5's fake-worker setup). The bundle lands only when the viewer panel is open, so the activation surface is unchanged. `CLAUDE.md` updated with shipping bullet + two new dead-end entries; `pdf2pptx/` reference tree deleted as part of the sign-off commit.
...
- **Phase D-thumbnail (2026-05-23) — in-file thumbnail on import.** `buildPptxFromImages` now writes `docProps/thumbnail.${ext}` using the first slide's encoded bytes and adds a `metadata/thumbnail` relationship to `_rels/.rels`. The viewer's existing `extractThumbnail` lookup (path-based, `/^docProps\/thumbnail\.<ext>$/`) picks it up unchanged. Re-uses the first-slide bytes verbatim (level: 0 store-only) rather than encoding a downsampled variant — keeps the build path simple at the cost of ~one slide-image-sized addition to the zip. This is the only place we ever write a thumbnail *into* a pptx; for any other file, see M-VE-3. Committed as `f64f794`.
- **Phase D-followup (2026-05-23) — fixed-height thumbnails + scoped cache replace on import.** Two cleanups surfaced from live PWA testing. (1) Thumbnails of different files rendered at different sizes — switched `.thumbnail` CSS from `max-width: 100%; max-height: 360px; height: auto` to `height: 240px; width: auto; max-width: 100%; object-fit: contain`. Every thumbnail now occupies the same vertical slot; landscape slides scale width by native aspect; portrait outliers stay 240px tall but narrower. (2) Re-importing the same PDF after the in-file-thumbnail change still surfaced the old (thumbnail-less) parse from the IDB cache, because the cache is keyed by sha256 and the new bytes' sha is unchanged from any prior cached import. Fix in `handleIngest` (picker path): call `cache.forget(candidate.sha256)` before `writeAndRender`. That evicts the single stale entry; the post-write `parsePptxCached` call inside `writeAndRender` then misses on that sha and records the fresh result via `cache.record`. Net effect is a scoped *replace* of one entry — every other file's cached parse is untouched. Log line: `ingest[picker]: evicted stale cache entry for sha256=… (will be repopulated by post-write re-parse)`. Committed as `553fa01` + log-rewording follow-up `b691c93`.
